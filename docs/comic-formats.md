...
PDFs are inspected with Poppler and rendered lazily, one requested page at a time, to a maximum 2400-pixel reader image. Encrypted/password-protected and malformed PDFs are rejected. Rendering has a 30-second timeout and uses a random, mode-0700 temporary directory that is removed after every attempt.

Archive inputs are limited to 10,000 entries and 2 GiB total reported uncompressed data. Only JPG, PNG, GIF, and WebP entries with safe, non-traversing names become pages. Page names are natural-sorted and never returned by the API.
