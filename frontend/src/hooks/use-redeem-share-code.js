...
function redemptionSummary(data) {
  const results = data.results || [];

  return {
    claimed: Number(data.claimed) || 0,
    gated: results.filter((result) => result.status === "awaiting_age_confirmation").length,
    emptyMessage: results.find((result) => result.message)?.message
      || "That code added nothing new to your collection.",
  };
}

function redeemedToast(data, { claimed, gated }) {
  const gatedDescription = gated === 1
    ? "One comic needs"
    : `${gated} comics need`;
  ...
  return {
    ...
    description: gated > 0
      ? `From ${data.ownerLabel}. ${gatedDescription} your age confirmed below.`
      : `${data.ownerLabel} shared ${claimed === 1 ? "a comic" : "these"} with you.`,
  };
}

async function refreshRedeemedComics(onRedeemed, toast) {
  try {
    await onRedeemed?.();
  } catch (refreshError) {
...
export function useRedeemShareCode(onRedeemed) {
  ...
  const redeem = async () => {
    ...
    try {
      ...
      const summary = redemptionSummary(data);
      ...
      if (summary.claimed === 0) {
        setError(summary.emptyMessage);
        return;
      }

      toast(redeemedToast(data, summary));

      setValue("");
      await refreshRedeemedComics(onRedeemed, toast);
    } catch (err) {
