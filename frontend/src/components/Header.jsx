...
export function Header({ isLoggedIn, onLogout, isAdmin }) {
  ...
  if (isReaderPage) {
    ...
    if (isFullscreen) {
      return (
        <header className="fixed top-0 left-0 right-0 z-30 bg-gradient-to-b from-background/90 to-transparent p-2 opacity-0 hover:opacity-100 transition-opacity duration-300">
      ...
      return (
        <header className="relative w-full bg-background border-b border-border py-1 px-4 z-30">
          <div className="flex justify-between items-center">
            <Link to="/dashboard" className="text-foreground hover:text-comic-purple text-sm">
              Back to Library
            </Link>
