 ...
 */
export function ReaderFitSuggestion({ fitLabel, contextLabel, message, acceptLabel = "Use it here", onAccept, onDismiss }) {
  return (
    <div
      role="status"
    ...
    >
      <p className="text-sm leading-snug">
        {message ?? <><span className="font-medium">{fitLabel}</span> reads better on {contextLabel}.</>}
      </p>
      <div className="flex shrink-0 items-center gap-1">
        <Button size="sm" variant="secondary" onClick={onAccept}>{acceptLabel}</Button>
